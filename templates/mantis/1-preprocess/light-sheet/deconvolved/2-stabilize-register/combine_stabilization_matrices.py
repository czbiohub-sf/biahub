...
reg_path = Path("../../raw/1-register/stabilization_settings.yml")
reg_settings = yaml_to_model(reg_path, StabilizationSettings)
reg_list = np.array(reg_settings.affine_transform_zyx_list)

# Folders with per-FOV Z and XY transform configs
z_dir = Path("../../raw/2-stabilize/z_stabilization_settings")
...
for z_file in sorted(z_dir.glob("*.yml")):
    ...
    if not (len(z_list) == len(xy_list) == len(reg_list)):
        raise ValueError(f"Transform list lengths mismatch for FOV {fov_name}")

    combined_list = [
        (reg @ z @ xy).tolist()
        for reg, z, xy in zip(reg_list, z_list, xy_list)
    ]
