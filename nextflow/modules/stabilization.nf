// Stabilization Nextflow module
//
// Provides 5 subworkflows for stabilization estimation and application:
//   - stabilization_estimate_focus_z_wf: fan-out Z-focus per position
//   - stabilization_estimate_focus_xyz_wf: 3-stage (Z focus → merge CSV → XY StackReg → combine)
//   - stabilization_estimate_pcc_wf: fan-out phase cross-correlation per position
//   - stabilization_estimate_beads_wf: single-shot beads estimation
//   - stabilize_wf: apply transforms (init + fan-out run × N positions)
//
...
process estimate_stabilization_z_focus {
    ...
    script:
    """
    ${biahub_cmd()} estimate-stabilization --step z-focus \
        -i "${in_zarr}/${position}" \
        -o "${out_dir}" \
        -c "${config}"
    """
...
process estimate_stabilization_xy {
    ...
    script:
    """
    ${biahub_cmd()} estimate-stabilization --step xy \
        -i "${in_zarr}/${position}" \
        -o "${out_dir}" \
        -c "${config}" \
        --focus-csv "${focus_csv}"
...
process estimate_stabilization_pcc {
    ...
    script:
    """
    ${biahub_cmd()} estimate-stabilization --step pcc \
        -i "${in_zarr}/${position}" \
        -o "${out_dir}" \
        -c "${config}"
    """
...
process estimate_stabilization_beads {
    ...
    script:
    """
    ${biahub_cmd()} estimate-stabilization --step beads \
        -i "${in_zarr}/${bead_pos}" \
        -o "${out_dir}" \
        -c "${config}"
    """
...
process run_stabilize {
    ...
    script:
    """
    ${biahub_cmd()} stabilize --cluster debug \
        -i "${in_zarr}/${position}" \
        -o "${out_zarr}" \
        -c "${config}"
    """
