...
# ─────────────────────────────────────────────────────────────────────────────
# Tile-stitch (Monarch actor-mesh engine) settings
# ─────────────────────────────────────────────────────────────────────────────
...
class MonarchConfig(BaseModel):
    ...
    per-position/timepoint ``swap_to``.
    """
    ...
    def _warn_prefetch_below_batch(self) -> Self:
        ...
        # buffered during the current FFT, so IO overlap is only partial. Warn,
        # don't raise. Checked on the EFFECTIVE depth, so a prefetch_batches
        # setting never trips it; only a raw prefetch_depth < recon_batch does.
...
class TileStitchReconSettings(MyBaseModel):
    """Top-level tile-stitch recon config: science + Monarch engine knobs + timepoints."""
    ...
    time_indices: int | list[int] | Literal["all"] = Field(
        default="all",
        description="Timepoints to reconstruct: 'all', a single index, or a list.",
    )
