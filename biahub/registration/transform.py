...
class Transform:
    ...
    def compose(self, other: Transform) -> Transform:
        ...
        if self.ndim != other.ndim:
            raise ValueError(
                f"Cannot compose {self.ndim}D and {other.ndim}D transforms"
            )
    ...
    def apply_points(self, points: NDArray[np.floating]) -> NDArray[np.floating]:
        ...
        if points.shape[1] != self._ndim:
            raise ValueError(
                f"points must have {self._ndim} columns, got {points.shape[1]}"
            )
    ...
    def to_ants(self):
        ...
        # Flatten the first 3 columns, then replace last 3 with translation
        ants_style = self._matrix[:, :-1].ravel()
        ants_style[-3:] = self._matrix[:3, -1]
        
        t_ants = ants.new_ants_transform(transform_type="AffineTransform")
        t_ants.set_parameters(ants_style)
        
        return t_ants
    
    @classmethod
    def from_ants(cls, ants_transform, transform_type: TransformType = "affine") -> Transform:
        """
        Create Transform from ANTs transform.
        
        Parameters
        ----------
        ants_transform : ants.ANTsTransform
            ANTs transform object.
        transform_type : TransformType
            Type to assign to the resulting transform.
        ...
        fixed_params = ants_transform.fixed_parameters
        
        # Reshape from ANTs format (from your original convert_transform_to_numpy)
        matrix = params.reshape((3, 4), order="F")
        matrix[:, :3] = matrix[:, :3].T
        matrix = np.vstack([matrix, [0, 0, 0, 1]])
        
        # Account for centering
        # T_final = T + (I - A) @ center
        matrix[:3, -1] += (np.eye(3) - matrix[:3, :3]) @ fixed_params
        
        return cls(matrix, transform_type=transform_type)
    ...
    def __str__(self) -> str:
        matrix_str = np.array2string(
            self._matrix, precision=4, suppress_small=True
        )
