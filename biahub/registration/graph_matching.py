...
class GraphMatcher:
    ...
    def _match_hungarian_cross_check(
        ...
        matches = np.array([
            [i, j] for i, j in matches_ab
            if (i, j) in reverse_map
        ], dtype=np.int32)
    ...
    def _compute_edge_consistency_cost(
        ...
        for i in range(n):
            ...
            for j in range(m):
                ...
                C_local = np.full(
                    (len(i_neighbors), len(j_neighbors)),
                    default_cost,
                    dtype=np.float32
                )
                ...
                for ii, ni in enumerate(i_neighbors):
                    for jj, nj in enumerate(j_neighbors):
                        ...
                        if mov_edge in mov_attrs and ref_edge in ref_attrs:
                            C_local[ii, jj] = abs(
                                mov_attrs[mov_edge] - ref_attrs[ref_edge]
                            )
    ...
    def _match_descriptor(
        ...
        if verbose:
            print(f"Matching {mov_desc.shape[0]} moving descriptors to {ref_desc.shape[0]} reference descriptors")
    ...
    def filter_matches(
        ...
        if min_distance_quantile is not None or max_distance_quantile is not None:
            dist = np.linalg.norm(
                moving.nodes[matches[:, 0]] - reference.nodes[matches[:, 1]],
                axis=1
            )
            ...
            if verbose:
                print(f"Distance filtering: quantiles [{min_distance_quantile}, {max_distance_quantile}]")
